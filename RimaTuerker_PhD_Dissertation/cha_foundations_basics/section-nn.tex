...
First we define error function $E(x)$ or cost function which measures the performance of the network as follows\todo{check formulas}:% from other book with y hat
...
\label{subsec:CNNs}
Convolutional neural networks (CNNs) are specialized neural networks for processing grid-structured data, e.g., image data (2D grid), time series data (1D grid). The \textit{convolutional} layer of CNNs is the most distinctive feature from other neural networks. Typically, a traditional CNN architecture would contain one or more convolutional layer. In 1998, the first convolutional neural network called \textit{LeNet-5} was developed by LeCun, Bottou, Bengio and Haffner. This network was trained with MNIST which is a large dataset contains binary images of handwritten digits. Since then CNNs have been utilized in variety of applications. In 2011 CNNs demonstrated remarkable performance in an image-classification contest. Thereafter, CNNs gained significant attention, especially, in the field of image processing. Although CNNs can be utilized with various type of data, the majority of the applications have been focused on image data. Some of the example of applications are image classification, image and video recognition, medical image analysis, etc.
%Although CNNs have been utilized in variety of applications since 1990s,

In 2014,~\citeauthor{DBLP:conf/emnlp/Kim14} designed a relatively simple CNN architecture for a sentence classification task\todo{check teh architecture and extnd this part}. In order to train the model Google pre-trained word vectors were used.\todo{heck this and cite} The model demonstrated strong empirical performance across several datasets. Today, the proposed model by Kim still the one of the most widely used baselines for text classification tasks. After the publication of the paper by ~\cite{DBLP:conf/emnlp/Kim14}, CNNs drawn attention from the NLP community as well. Several scientific literature have been published in the similar direction~\cite{}.
...
The convolutional neural networks are similar to the traditional feed forward neural networks where the information moves in one direction, forward, from input layer to hidden layers and finally the output layer. Figure~\ref{fig:cnn_text_cat} illustrates a  simple example of a CNN architecture for a sentence classification task. Unlike to feed forward neural networks which typically contains one type of a hidden layer i.e, fully connected layer, convolutional neural networks consist of different type of layers.  The most common three type of layers that present in standard convolutional neural networks are \textit{convolution}, \textit{pooling},and \textit{ReLU} layer. ReLU is an activation layer which is similarly applied as in the traditional neural networks. Finally, the final layers of convolutional neural networks  are often fully connected. %  which maps given inputs into the set of output nodes. 

Before discussing each layer of convolutional neural networks, first, we briefly examine general architecture of the network (cf. Figure~\ref{fig:cnn_text_cat}). The majority of the related literature focused on analyzing the model in setting of image classification where the input is image data. However, since the main topic of this thesis is text categorization we discuss model in the context of text categorization. %Therefore, we assume that our input is text data. 
Figure illustrates a simple example of a convolutional neural network architecture which is designed for sentence classification. The goal of the model is a piece of text, i.e., a sentence, to assign a label or category to it. As it has already mentioned, convolutional neural networks accept grid-like structured data as an input, therefore, each input sentence converted to a sentence matrix. To that end, each given sentence is tokenized.% and rows of the matrix are the vector representation of each token.
The rows of the matrix are the vector representation of each token. These might be e.g., word vectors from a pre-trained Skip-gram model or from any other word embedding model. Let the $d$ denote the dimension of the word vectors and $s$ is the number of tokens present in a given sentence. Then, the dimensionality of the sentence matrix is $s\times d$. The value of $s$, i.e., the number of words to be considered from an input sentence to form a sentence matrix is often predefined. Because, the dimension of the input matrix determines the number of neurons in the input layer. To define the value of $s$, often, the maximum length of the sentence in a given dataset is considered. Definitely, the length of the sentences in standard datasets may vary and thus the zero-\textit{padding} strategy is used to equalize the dimension of the input matrices.  
Given a sentence matrix the next operation is convolution which is performed via \textit{"filters"}. The number and the size of the filters determined based on the application at hand. In Figure~\ref{fig:cnn_text_cat} there are 6 different filters with 3 different sizes. After the convolution operation the obtained feature maps have different dimensions. In order to extract a scalar from each feature map and induce them to a fixed length vector max pooling function is applied. 
...
Finally, those vectors are concatenated to form a single feature vector. To produce the outputs, this vector can be fed through a fully connected layer and then a softmax function or directly to a softmax function. \\
 %or to a fully connected layer then to a softmax function to produce the outputs. 
In the next subsections the details of each layer have been\todo{check grammer} discussed.  
